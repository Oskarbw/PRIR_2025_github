...
class Strength:
    ...
    def highest_stress(self, diameters) -> Optional[float]:
        ...
        deegres_of_freedom,
        deegres_of_constraint) = self._assign_boundary_conditions(nodes, dirichlet_neumann)

        K = self._assemble_stiffness_matrix(node_deegres_of_freedom_global,elements,nodes,diameters)

        (final_displacements,
        final_forces) = self._solve_system(K,
                                           dirichlet_neumann,
                                           initial_displacements,
                                           initial_forces,
                                           deegres_of_freedom,
                                           deegres_of_constraint,
                                           nodes)
    ...
    def _solve_system(self, K, dirichlet_neumann, initial_displacements, initial_forces, deegres_of_freedom, deegres_of_constraint, nodes):
        ...
        (final_displacements, final_forces) = self._order_results(dirichlet_neumann, final_displacements_unordered, final_forces_unordered, nodes)

        return final_displacements, final_forces
    ...
    def _order_results(self, dirichlet_neumann, final_displacements_unordered, final_forces_unordered, nodes):
        num_nodes= np.size(nodes,0)
    ...
    def _calculate_highest_stress(self, displacements, elements, nodes):
        ...
        for element in elements:
            ...
            # Długość po uwzględnieniu przemieszczeń węzłów
