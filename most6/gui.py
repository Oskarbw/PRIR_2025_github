...
class OptimizationApp:
    ...
    def _show_results(self, bridge):
        ...
        highest_stress = bridge.highest_stress / constants.MEGA
        elastic_limit = constants.ELASTIC_LIMIT_OF_STEEL / constants.MEGA
        elapsed_time = self.end_time - self.start_time

        self.mass_var.set(f"{mass:.2f} kg")
        self.highest_stress_display_var.set(f"{highest_stress:.4f} MPa")
        self.elastic_limit_display_var.set(f"{(elastic_limit):.2f} MPa")
